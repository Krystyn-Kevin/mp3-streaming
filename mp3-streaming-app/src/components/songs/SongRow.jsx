import { Heart, Plus, ListPlus, Pause, Trash2, Loader2 } from "lucide-react";
...
const GRID_COLS = "md:grid-cols-[40px_minmax(0,2.4fr)_minmax(0,1.4fr)_minmax(0,1.4fr)_minmax(0,1.4fr)_70px_112px]";
...
export default function SongRow({ song, index, songsInView, isCurrent }) {
  ...
  const { deleteSong, deletingId } = useDeleteSong();
  ...
  function handleDelete(e) {
    e.stopPropagation();
    if (window.confirm(`Remove "${song.title}" from the library? This can't be undone.`)) {
      deleteSong(song);
    }
  }
