...
export const usePlayerStore = create(
  persist(
    (set, get) => ({
      ...
      /** Removes every queue entry for a given song id — used when a song
       * is deleted from the library entirely, since it may appear in the
       * queue more than once. */
      removeSongEverywhere: (songId) => {
        set((state) => {
          const currentKey = state.currentIndex >= 0 ? state.queue[state.currentIndex]?.__queueKey : null;
          const queue = state.queue.filter((s) => s.id !== songId);
          let currentIndex = currentKey ? queue.findIndex((s) => s.__queueKey === currentKey) : -1;
          if (currentIndex === -1) currentIndex = Math.min(state.currentIndex, queue.length - 1);
          return { queue, currentIndex, isPlaying: queue.length > 0 && state.isPlaying };
        });
      },
